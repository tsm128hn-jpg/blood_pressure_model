...
# ==========================================
# 3. HARDWARE SIMULATOR (Replace with Arduino later)
# ==========================================
def get_sensor_data():
    """
    For the software demo, we simulate the Arduino sending a PPG wave and 5 features.
    When you connect the real MAX30102, replace this with PySerial reading from the COM port.
    """
    # Simulate a 4-second normal PPG wave segment (512 points)
    ...
    # Simulate the 5 extracted features: [Mean, Std, AUC, HR, HRV]
    # Simulating a heart rate of ~72 BPM
...
# ==========================================
# Initialize monitoring state in session
...
if st.session_state.is_monitoring and model is not None:
    ...
    # Use a placeholder for streaming updates
    placeholder = st.empty()
    
    # Simulate continuous monitoring with updates
    for _ in range(5):  # Collect 5 readings per session
        # 1. Fetch data from sensor
        seq, tab = get_sensor_data()
        
        # 2. AI predicts baseline
        preds = model.predict({"input_sequence": seq, "input_tabular": tab}, verbose=0)
        raw_sys = preds[0][0][0]
        raw_dia = preds[0][0][1]
        
        # 3. APPLY CALIBRATION MAGIC
        final_sys = raw_sys + st.session_state.sys_offset
        final_dia = raw_dia + st.session_state.dia_offset
        
        # Add slight natural physiological variance for the live simulation
        ...
        # Keep only the last 50 readings to prevent memory overflow
    ...
    # Display historical data when not monitoring
